...
export default async function RosterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [gamesRaw, playersRaw] = await Promise.all([
    fetchJson(`games?id=eq.${id}&select=id,game_date,opponent_id,home_away,video_urls`),
  ...
  const players: RosterPlayer[] = Array.isArray(playersRaw) ? playersRaw : []
      ...
      <div className="px-4 md:px-8 py-5" style={{ background: HEADER, borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 920, margin: '0 auto' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800 }}>
            {game ? <>vs {opponentName}</> : 'Game not found'}
          </div>
          {game && (
            <div style={{ fontSize: 13, color: MUTED, marginTop: 4 }}>
              {game.game_date} · {game.home_away ?? '—'}. Check who dressed, then pick your starting five.
            </div>
          )}
        </div>
      </div>
