import WatchScreen, { type WatchPlayer } from './WatchScreen'
...
// Map stored play_by_play rows to the LocalEvent shape the review player replays.
// clock_time / video_time are text columns → coerce back to numbers.
/* eslint-disable @typescript-eslint/no-explicit-any */
function toEvents(pbp: any[]): LocalEvent[] {
...
export default async function WatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const players: WatchPlayer[] = Array.isArray(playersRaw) ? playersRaw : []
      ...
      <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: BG, fontFamily: "'Inter', system-ui, sans-serif" }}>
        <div style={{ textAlign: 'center', color: '#6b7280', fontSize: 13 }}>
          {!game
            ? <>Game not found.</>
            : videoUrls.length === 0
            ? <>This game has no video attached — review mode needs the game footage.</>
            : <>No play-by-play captured for this game yet.</>}
